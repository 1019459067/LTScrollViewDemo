...
public class LTPageView: UIView {
    ...
    @objc public var titles: [LTButtonModel] {
        didSet {
            for i in 0..<titles.count {
                let btnModel:LTButtonModel = titles[i]
                let btn : UIButton = glt_buttons[i]
                btn.setTitle(btnModel.title, for: .normal)
                if btnModel.showIcon {
                    btn.setImage(UIImage(named: btnModel.normalImageName), for: .normal)
                    btn.setImage(UIImage(named: btnModel.selectedImageName), for: .selected)
    ...
    @objc public init(frame: CGRect, currentViewController: UIViewController, viewControllers:[UIViewController], titles: [LTButtonModel], layout: LTLayout) {
...
extension LTPageView {
    ...
    private func buttonsLayout() {
        ...
        // 将所有的宽度计算出来放入数组
        for btnModel in titles {
            let text = btnModel.title
        ...
        for index in 0..<titles.count {
            let btnModel = titles[index]

            let subW = glt_textWidths[index]
            let button = subButton(frame: CGRect(x: upX, y: subY, width: subW, height: subH), flag: index, title: btnModel.title, parentView: sliderScrollView)
    ...
    private func setupTitleSelectIndex(_ btnSelectIndex: Int) {
        ...
        updateButtonUIStatus(index: btnSelectIndex)
    }
...
extension LTPageView {
    ...
    private func scrollViewDidScrollOffsetX(_ offsetX: CGFloat)  {
        ...
        if glt_currentIndex != index {
            ...
            updateButtonUIStatus(index: index)
        }
    ...
    // 更新按钮等状态
    func updateButtonUIStatus(index:NSInteger) {
        for i in 0..<glt_buttons.count {
            let btn : UIButton = glt_buttons[i]
            btn.isSelected = index == i ? true:false
        }
    }
